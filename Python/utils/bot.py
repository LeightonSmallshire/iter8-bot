...
class Users:
    ...
    @staticmethod
    def random(exclude: list[int] | None = None) -> int:
...
def is_guild_paradise(ctx: discord.Interaction) -> bool:
    guild = ctx.guild
    return guild is not None and guild.id == Guilds.Default
...
async def is_user_role(ctx: discord.Interaction, role_id: int) -> bool:
    ...
    return role in (member.roles if member is not None else [])
...
async def on_new_admin(interaction: discord.Interaction, new_admin: int) -> None:
    ...
    if officers is not None:
        ...
        for officer in officers.members:
            await officer.remove_roles(officers, reason="New admin selected")
            await officer.add_roles(default_role, reason="Reset to default role")

async def do_role_roll(
    ...
    message_contents = (
        response[0].format(prev_user.id, choice) if prev_user else response[1].format(choice)
    )
