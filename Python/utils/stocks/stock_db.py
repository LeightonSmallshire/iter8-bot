...
async def get_all_stocks() -> list[Stock]:
    async with Database(DATABASE_NAME) as db:
        return cast(list[Stock], await db.select(Stock))
...
async def can_afford_stock(user_id: int, stock_id: str, count: int) -> tuple[bool, str | None]:
...
async def stock_market_short(user_id: int, stock_id: str, count: int, auto_sell_low: datetime.timedelta | None, auto_sell_high: datetime.timedelta | None) -> tuple[bool, str]:
    async with Database(DATABASE_NAME) as db:
        stocks = await db.select(Stock, where=[WhereParam("code", stock_id.upper())])
        if not stocks:
...
async def stock_market_update_trade(user_id: int, trade_id: int, auto_sell_low: datetime.timedelta | None, auto_sell_high: datetime.timedelta | None) -> tuple[bool, str]:
    async with Database(DATABASE_NAME) as db:
        orders = await db.select(Trade, where=[WhereParam("id", trade_id), WhereParam("user_id", user_id), WhereParam("sold_at", None, "IS")])
        if not orders:
